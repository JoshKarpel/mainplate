---
description: "The stylesheet, the script and the vendored font: the three shapes, the monospace grid, and what mainplate.js is responsible for."
---

# The assets

`mainplate.css`, `mainplate.js`, the vendored `htmax.min.js`, and the two JuliaMono weights. The
page these draw is [the console](https://joshkarpel.github.io/mainplate/design/console/); this is
what to know while editing the files themselves.
...
looked at rather than argued about. `just serve` restarts the server on any change under
`src/mainplate`, which is what makes an edit here watchable: the assets are inventoried once at
startup, so an edited stylesheet only reaches a *new* process.

**Everything the script does is an enhancement.** With `mainplate.js` absent the page still renders,
posts, and folds. What the rail projects onto the transcript, the search marks, the panel landed on,
which kinds are muted, what is folded, cannot live in the markup, so the script holds it as values
and reapplies it after every swap through one idempotent `repaint()` serving the first render, every
swap, and every press.
...
applies at two widths (the rail's overlay) stays with the thing it is about.
...
**And it is `html { font-size }`.** Everything here except the monospace grid is sized in `rem`, the
...
`htmax.min.js` is htmx 4 core plus every bundled extension in one file, and which extensions are
actually installed is an allowlist in a meta tag the server renders from `EXTENSIONS` in `pages.py`.
Do not replace it with core plus separately vendored extensions; the reasoning is on [the console's
page](https://joshkarpel.github.io/mainplate/design/console/#htmx-4).
