...
def test_this_repository_declares_its_setup_plugin_at_the_path_it_is_actually_at() -> None:
    """
    ...
    assert declared.name == "setup"
    assert declared.path.is_file(), "setup is declared at a path with no file at it"
    assert os.access(declared.path, os.X_OK), "setup is not executable, so nothing can run it"
