...
#
# No server, no database, no provider and no `config.toml`. Pages are pure functions of
...
OFFERED = {
    "llm-anthropic": (
        Listed(id="anthropic/claude-sonnet-4-6", label="Claude Sonnet 4.6", family="anthropic"),
        Listed(id="anthropic/claude-opus-4-8", label="Claude Opus 4.8", family="anthropic"),
        Listed(id="fireworks/kimi-k2", label="Kimi K2", family="fireworks"),
    ),
    "llm-openai": (Listed(id="openai/gpt-5.5", label="openai/gpt-5.5", family="openai"),),
}

CATALOGUE = Catalogue(
    offered=OFFERED,
    default=Choice(profile="llm-anthropic", model="anthropic/claude-sonnet-4-6", thinking="high"),
)
...
def showing(session: Session, written: dict[str, object], *, answerable: bool = True) -> Conversation:
    return Conversation(
        session=session,
        said=transcript(snapshotted(written)),
        chosen=CATALOGUE.default,
...
def pages() -> dict[str, str]:
    ...
    return {
        ...
        # Branching at turn 1, so the page has something to show as carried over and something to
        # leave behind: the fork keeps turn 0 and waits to be told turn 1 differently.
        "forking.html": fork_page(LINKS, LISTED, showing(PARENT, settled), 1, CATALOGUE),
        # And at turn 0, which is the sentence that reads wrong if it is written carelessly.
        "forking-start.html": fork_page(LINKS, LISTED, showing(PARENT, settled), 0, CATALOGUE),
    }
